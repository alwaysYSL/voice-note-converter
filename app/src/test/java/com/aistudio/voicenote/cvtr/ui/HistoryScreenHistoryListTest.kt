...
class HistoryScreenHistoryListTest {
    ...
    fun `history exposes search filter and sort controls`() {
        ...
        composeRule.onNodeWithText("Belum dikonfirmasi").assertExists()
        composeRule.onNodeWithText("Dikonfirmasi").assertExists()
    ...
    fun `search query hides nonmatching rows`() {
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodes(hasText("Meeting.m4a")).fetchSemanticsNodes().isNotEmpty() &&
                composeRule.onAllNodes(hasText("Ide.m4a")).fetchSemanticsNodes().isEmpty()
        }
    }
